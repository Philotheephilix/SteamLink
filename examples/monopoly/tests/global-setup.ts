 ...
 * setup no longer spawns a separate server/bots process — it only ensures the player
 * keys exist and are funded BEFORE the app boots (globalSetup runs before the
 * webServer starts), so:
 *   - players.local.json exists with stable keys (the test injects the human seat-0
 *     key into the browser's localStorage guest wallet), and
 *   - auto-start reuses those funded keys instead of regenerating under load.
 *
 * The wait for a seated game happens in the spec's beforeAll (after the webServer is
 * up), since the game is created by the app itself.
 */
...
export default async function globalSetup() {
  ...
  // A bounded 2-player game (human + 1 bot) reaches a real last-solvent finish within
  // the test budget. The app's auto-start reads this too.
